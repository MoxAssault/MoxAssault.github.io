(() => {
  ...
  async function generateReadme(mode, button = null) {
    if (generating) return;

    const { record, values } = currentContext();
    if (!record || !values) {
  ...
  window.VPS_README_GENERATOR = Object.freeze({
    getContext: currentContext,
    buildManualReadme,
    buildWizardReadme,
    generate: mode => generateReadme(mode)
  });
