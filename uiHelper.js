...
function renderGameCard(record, rawID) {
  const groupKeys = [
    'tableFiles',
    'b2sFiles',
    'romFiles',
  ...
  manufBox.classList.add('meta-box', 'meta-left');
  ...
  updatedBox.classList.add('meta-box', 'meta-middle');
  const updatedAt = record.updatedAt ? (new Date(record.updatedAt)).toLocaleDateString(undefined, {year:'numeric',month:'short',day:'numeric'}) : '';
  updatedBox.textContent = updatedAt ? `Updated: ${updatedAt}` : '';
  const idBox = document.createElement('div');
  idBox.classList.add('meta-box', 'meta-right');
